    ...
    while True:
        ...
        for i in index:
            try:
                ...
                im, text_polys = resize_image(im, text_polys, FLAGS.input_size, shift_h, shift_w)
                new_h, new_w, _ = im.shape
                score_map, geo_map = generate_rbox(
                    FLAGS, (new_h, new_w), text_polys, text_tags)
                ...
                if len(images) == FLAGS.batch_size:
                    yield [np.array(images), np.array(score_maps)], [np.array(score_maps), np.array(geo_maps)]
                    images = []
                    image_fns = []
                    score_maps = []
                    geo_maps = []
...
def load_data_process(FLAGS, image_file, is_train):
    try:
        ...
        new_h, new_w, _ = img.shape
        score_map, geo_map = generate_rbox(FLAGS, (new_h, new_w), text_polys, text_tags)
        img = (img / 127.5) - 1.
        return img[:, :, ::-1].astype(np.float32), image_file, score_map[::4, ::4, np.newaxis].astype(
            np.float32), geo_map[::4, ::4, :].astype(np.float32)
    except Exception as e:
        import traceback
        traceback.print_exc()
...
def load_data(FLAGS, is_train=False):
    ...
    return np.array(images), np.array(score_maps), np.array(geo_maps)
